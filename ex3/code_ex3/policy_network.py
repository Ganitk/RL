...
class PG(object):
    ...
    def add_placeholders_op(self):
        ...
        t = tf.int32 if self.discrete else tf.float32
        shape_a = (None,) if self.discrete else (None,self.action_dim)
        self.action_placeholder = tf.placeholder(t, shape=shape_a)
    ...
    def build_policy_network_op(self, scope="policy_network"):
        ...
        if self.discrete:
            mlp_net = build_mlp(self.observation_placeholder, self.action_dim, scope, self.config.n_layers,
                                self.config.layer_size, self.config.activation)
            self.sampled_action = tf.squeeze(tf.multinomial(mlp_net,1))
            ...
            self.sampled_action = tf.random_normal(shape=(1,), mean=mlp_net, stddev=log_std )
            dist = tf.contrib.distributions.MultivariateNormalDiag(loc=mlp_net, scale=log_std)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            # mask = np.array([self.config.gamma ** i for i in range(T)])
            # returns = np.zeros(T)
            # for r in range(T):
            #     returns[r] = np.sum([rewards[i+r] + self.config.gamma ** i for i in range(T-r)])
